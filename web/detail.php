...
function permission($conn,$namaPerusahaan){
    $companyTable = new companyTable($conn);
    if ($_SESSION["tipeUser"]=="company"){
        $currentnama = $companyTable->getCompanyByEmail($_SESSION['email'])->getNamaPerusahaan();
...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (isset($_POST["hapusLowongan"])) {
        if ($_POST["hapusLowongan"] == "hapus" && permission($conn,$namaperusahaan)){
            if (count($cvTable->getAllCV($namaperusahaan,$jobdesk)) > 0){
                echo "<script>alert('Sudah Ada Pelamar, Tidak Dapat Di Hapus');</script>";
            }else{
